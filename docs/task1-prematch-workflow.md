...
2. Resolve competitions by API-Football league ID first and normalized exact aliases second. Fetch current-season Brasileirão Série A membership from league ID 71; any match with one member club is selected, including Libertadores (13), Sudamericana (11), and other competitions. National-team families (UEFA/Concacaf/Conmebol Nations League, World Cup and its qualifiers, Euro Championship and qualifiers, Copa América, Gold Cup, Asian Cup, Africa Cup of Nations, friendlies) are also selected by competition name, because the agenda feed leaves `league_id` null and `league_country` empty for those rows; a national-team fixture is published only when a Brazilian outlet actually carries it, so YouTube-only rows stay out of the paid slate unless `REQUIRE_BRAZILIAN_BROADCAST_FOR_NATIONAL_TEAMS` is set to `False`. Save the membership and selection reason beside `phase1/selected-fixtures.json` for Task 3.
