...
1. Collect tomorrow fixtures from the configured `https://copa.jarg.top/jogos-de-hoje` collector, using Brasília calendar time only. Cross-check fixture data with the configured API-Football route when available; API keys must come from env/Keychain, never from committed text.
...
6. Generate only the first 3-4 second dynamic poster hook with the configured Dreamina/Jimeng VIP Seedance route. Middle operation clips, CTA, music, and reusable voice inventory come from local authorized assets and are stitched into the 12-second final video.
...
9. Build `captions.json` in pt-BR. Each caption must contain match information plus JaguarTV marketing, include `Jaguar TV`/`jaguartvbrasil.com` style CTA, and use exactly five hashtags with `#jaguartvbrasil` last.
